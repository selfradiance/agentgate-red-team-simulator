// Integration tests for authorization boundary attack scenarios — requires live AgentGate

import "dotenv/config";
import { describe, it, expect, beforeAll } from "vitest";
import { generateKeyPairSync, randomUUID } from "node:crypto";
import { authorizationAttacks } from "../../src/attacks/authorization";
...
describe.skipIf(!process.env.AGENTGATE_REST_KEY || process.env.AGENTGATE_REST_KEY.includes("your-"))(
  "authorization attacks — live AgentGate",
  () => {
    ...
    it("4.1 — admin endpoint without admin key is rejected", async () => {
      const result = await authorizationAttacks[0].execute(client);
      expect(result.caught).toBe(true);
      expect(result.scenarioId).toBe("4.1");
    });

    it("4.2 — resolve another identity's action is rejected", async () => {
      const result = await authorizationAttacks[1].execute(client);
      expect(result.caught).toBe(true);
      expect(result.scenarioId).toBe("4.2");
    });
  },
);

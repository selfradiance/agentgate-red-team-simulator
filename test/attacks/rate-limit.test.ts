// Integration tests for rate limit attack scenarios — requires live AgentGate

import "dotenv/config";
import { describe, it, expect, beforeAll } from "vitest";
import { generateKeyPairSync, randomUUID } from "node:crypto";
import { rateLimitAttacks } from "../../src/attacks/rate-limit";
...
describe.skipIf(!process.env.AGENTGATE_REST_KEY || process.env.AGENTGATE_REST_KEY.includes("your-"))(
  "rate limit attacks — live AgentGate",
  () => {
    ...
    it("6.1 — execution rate limit is enforced", { timeout: 15000 }, async () => {
      const result = await rateLimitAttacks[0].execute(client);
      expect(result.caught).toBe(true);
      expect(result.scenarioId).toBe("6.1");
    });
  },
);
